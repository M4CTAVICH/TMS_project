...
const mountainUniforms = {
  uFreq: { value: new THREE.Vector4(3, 4, 8, 1) },
  uAmp: { value: new THREE.Vector4(10, 5, 50, 10) },
};
...
const deepUniforms = {
  uFreq: { value: new THREE.Vector4(4, 8, 8, 1) },
  uAmp: { value: new THREE.Vector4(30, 5, 20, 20) },
  ...
  mountains: {
    ...
    getJS: (progress: number, time: number) => {
      ...
      const getX = (p: number) =>
        ...
        Math.pow(Math.cos(Math.PI * p * uFreq.y * (uFreq.y / uFreq.x)), 2) *
          uAmp.y;
...
function resizeRendererToDisplaySize(
  ...
  // Prevent rendering with zero dimensions
  if (width <= 0 || height <= 0) {
    return false;
  }
...
class App {
  ...
  onWindowResize() {
    ...
    const width = Math.max(this.container.offsetWidth || 1, 1);
    const height = Math.max(this.container.offsetHeight || 1, 1);
  ...
  setSize(width: number, height: number, updateStyles: boolean) {
    ...
    this.composer.setSize(validWidth, validHeight, updateStyles);
  }

  tick() {
    if (this.disposed) return;

    // Check if container still has valid dimensions
